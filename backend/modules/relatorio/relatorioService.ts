...
export const relatorioService = {
  async create(data: CreateRelatorioDTO): Promise<Relatorio> {
    if (
      ...
      throw new Error(
        "id_solicitante, id_atleta, tipo_relatorio, formato, periodo_inicio e periodo_fim são obrigatórios",
      );
    }
  ...
  async getByEquipe(id_equipe: number): Promise<Relatorio[]> {
    return relatorioRepository.findByEquipe(id_equipe);
  },
