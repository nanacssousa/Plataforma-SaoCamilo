...
export default function PosSessaoScreen() {
  ...
  const handleFinalizar = async () => {
    ...
    if (!peso || isNaN(pesoNum) || pesoNum <= 0) {
      Alert.alert(
        "Peso obrigatório",
        "Informe seu peso pós-treino para calcular a desidratação.",
      );
      return;
    }
    if (!sessaoAtiva) {
      Alert.alert(
        ...
        "Não foi possível encontrar uma sessão iniciada. Por favor, inicie uma nova sessão pela tela de Pré-Sessão.",
        [{ text: "OK", onPress: () => router.replace("/presessao") }],
      );
      return;
    }
    setFinalizando(true);
    try {
      ...
      Alert.alert(
        "Erro inesperado",
        "Ocorreu um erro ao salvar a sessão. Tente novamente.",
      );
    } finally {
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      {sessaoAtiva && (
        ...
        >
          ...
          >
            SESSÃO ENCERRADA • {sessaoAtiva.tipoTreino} • {sessaoAtiva.aguaML}ml
            registrados
          </Text>
        </View>
      )}
          ...
          {SYMPTOMS.map((s) => (
            <TouchableOpacity
              ...
              style={{
                ...
                borderRadius: 10,
                alignItems: "center",
                backgroundColor: sintomas.includes(s.key)
                  ? colors.primary
                  : colors.surfaceContainerLow,
                borderWidth: 1.5,
                borderColor: sintomas.includes(s.key)
                  ? colors.primary
                  : colors.outlineVariant,
              }}
      ...
      >
        <TouchableOpacity
          style={{
            backgroundColor: finalizando ? colors.gray400 : colors.primary,
            borderRadius: 10,
            paddingVertical: 16,
            alignItems: "center",
            flexDirection: "row",
        ...
        >
          ...
          >
            {finalizando ? "SALVANDO..." : "FINALIZAR E VER RESULTADO"}
          </Text>
          {!finalizando && (
