          ...
          <View style={styles.graficoBarras}>
            <View
              style={[
                styles.barra,
                styles.barraUsg,
                { height: ALTURA_MAX * Math.min(item.usgBar, 1) },
              ]}
            />
            <View
              style={[
                styles.barra,
                styles.barraMassa,
                { height: ALTURA_MAX * Math.min(item.massaBar, 1) },
              ]}
            />
          </View>
...
function normalizarParaGrafico(sessoes: any[]): DiaGrafico[] {
  ...
  return DIAS_SEMANA.map((dia, i) => {
    ...
    const sessoesDoDia = sessoes.filter(
      (s) => (s.data_treino ?? "").slice(0, 10) === dataStr,
    );
    if (sessoesDoDia.length === 0) return { dia, massaBar: 0, usgBar: 0 };
    const mediaPerdaMassa =
      sessoesDoDia.reduce(
        (acc: number, s: any) => acc + Math.abs(s.perda_percentual_massa ?? 0),
        0,
      ) / sessoesDoDia.length;
    const mediaUsg =
      sessoesDoDia.reduce((acc: number, s: any) => acc + (s.usg ?? 1.015), 0) /
      sessoesDoDia.length;
    return {
      dia,
      massaBar: Math.min(mediaPerdaMassa / 5, 1), // normaliza até 5% de perda
      usgBar: Math.min((mediaUsg - 1.0) / 0.03, 1), // normaliza de 1.000 a 1.030
    };
  });
}
...
export default function TelaEquipes() {
  ...
  const [grafico, setGrafico] = useState<DiaGrafico[]>(
    DIAS_SEMANA.map((dia) => ({ dia, massaBar: 0, usgBar: 0 })),
  );
  ...
  const carregar = useCallback(async (silencioso = false) => {
    ...
    try {
      ...
      if (Array.isArray(atletasRaw)) {
        ...
        const hidratados = atletasMapped.filter(
          (a) => a.statusHidrico === "hidratado",
        ).length;
        setMediaHidratacao(
          atletasMapped.length > 0
            ? Math.round((hidratados / atletasMapped.length) * 100)
            : 0,
        );
      }
      ...
      Alert.alert(
        "Erro",
        err.message ?? "Não foi possível carregar os dados da equipe.",
      );
    } finally {
      setLoading(false);
      setRefreshing(false);
    ...
    : [
        {
          id: "1",
          tipo: "ajuste",
          titulo: "Equipe dentro dos parâmetros",
          descricao:
            "Manter oferta hídrica atual e monitorar USG após o treino.",
        },
      ];
            ...
            <View style={styles.kpiColuna}>
              <View style={[styles.card, styles.cardKpiRisco]}>
                ...
                <Text style={styles.kpiNumero}>
                  {String(atletasEmRisco).padStart(2, "0")}
                </Text>
              ...
              <View style={[styles.card, styles.cardKpiHid]}>
                ...
                <Text style={styles.kpiPorcentagem}>
                  {mediaHidratacao}
                  <Text style={styles.kpiPorcentagemSufixo}>%</Text>
                </Text>
          ...
          <View style={styles.rodape}>
            ...
            <View style={[styles.card, styles.cardRelatorio]}>
              ...
              <Text style={styles.relatorioDesc}>
                {atletas.length} atletas monitorados. Desempenho hídrico:{" "}
                {mediaHidratacao}%.
              </Text>
              <TouchableOpacity
